...
const RandevuDetay = () => {
  ...
  useEffect(() => {
    api
      .get(`${urls.randevular}/${randevuId}`)
      .then((randevuRes) => {
        setRandevu(randevuRes.data);
        api
          .get(`${urls.hastalar}/${randevuRes.data.hastaId}`)
          .then((hastaRes) => {
            ...
            api
              ...
              .then((islemlerRes) => {
                ...
                let myIslemler = [];
                for (let i = 0; i < hastaRes.data.islemIds.length; i++) {
                  for (let j = 0; j < islemlerRes.data.length; j++) {
                    if (hastaRes.data.islemIds[i] === islemlerRes.data[j].id) {
              ...
              .catch((islemlerErr) => console.log(islemlerErr));
          })
          .catch((hastaErr) => console.log(hastaErr));
      })
      .catch((randevuErr) => console.log(randevuErr));
  }, []);
  ...
  return (
    <div>
      ...
      <h4>Telefon Numarası: {hasta.phone}</h4>
